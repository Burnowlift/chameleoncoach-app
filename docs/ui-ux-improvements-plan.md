...
| **F1 — Timeline do treinador** (2.3) | ⏳ Pendente | Continua sendo navegação bloco → semana |
| **F2 — Gráficos** | ⏳ Pendente | `RmEvolutionChart` mantido; novos gráficos (volume/RPE/aderência) não iniciados |
| **F3 — Gamificação** | ⏳ Pendente | Não iniciada |
...
| **F4 — Background Sync** | ⏳ Futuro | Fila é drenada no evento `online`; registro de `sync.register` fica para depois |

**Pendências de segurança (fora deste plano):** CLI do Supabase instalado (`supabase` v2.111.0), mas as 5 migrations (`20260806120000` a `20260806120400`) e o deploy das Edge Functions ainda **não foram aplicados** — aguardando `supabase login` + `supabase db push` + `supabase functions deploy`.
