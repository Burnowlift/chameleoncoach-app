...
| **F4 — Medalhas com tabela no banco** | ✅ Implementado | Tabela `user_achievements` (migração `20260807010000` aplicada) + `useAchievements`: derivada no cliente e desbloqueios persistidos automaticamente (upsert idempotente), com data de desbloqueio exibida no grid; sincroniza entre dispositivos |
...
| **F4 — Web Push (Opção A do plano mobile)** | ✅ Ativo | SW `push`/`notificationclick`, tabela `push_subscriptions`, Edge Function `register-push-subscription`, toggle no sino, VAPID keys + `scripts/send-push.js` — **deployado em 07/08** |

**Pendências de segurança (fora deste plano):** ✅ **RESOLVIDO em 07/08** — `supabase db push` aplicado (migrations `20260805170000`–`20260807000000`; as duas de bucket/policies que já existiam manualmente foram tornadas idempotentes com `DROP POLICY IF EXISTS`) e `supabase functions deploy` concluído (8 funções, incluindo `register-push-subscription`). Políticas RLS verificadas no banco.
