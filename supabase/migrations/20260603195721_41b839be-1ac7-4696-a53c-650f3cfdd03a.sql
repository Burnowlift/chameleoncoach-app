...
RETURNS TABLE (
  ...
  penalty numeric,
  penalties jsonb,
  load_fill_rate numeric,
  mobility_rate numeric,
  message_rate numeric,
  expected_load_logs int,
  actual_load_logs int,
...
BEGIN
  ...
  WITH
  base AS (
    SELECT s.id AS sid, s.name AS sname, s.avatar AS savatar
  ...
  computed AS (
    SELECT
      b.sid, b.sname, b.savatar,
      GREATEST(0, ROUND(COALESCE(ba.expected_sessions_per_week,0) * COALESCE(ba.avg_ex_per_session,0)))::int AS expected_loads,
      COALESCE(ll.actual_loads, 0)::int AS actual_loads,
      COALESCE(mi.items, 0) * WINDOW_DAYS AS expected_mob,
  ...
  SELECT
    ...
    f.sname,
    f.savatar,
    GREATEST(1.0, ROUND((f.base_score_calc - f.total_penalty)::numeric, 1)),
    f.base_score_calc,
    f.total_penalty,
    f.penalties_jsonb,
    f.load_rate,
    ...
    f.expected_msgs,
    f.actual_msgs,
    ROW_NUMBER() OVER (ORDER BY GREATEST(1.0, ROUND((f.base_score_calc - f.total_penalty)::numeric, 1)) DESC, f.sname ASC)::int
  FROM finalc f
  ORDER BY 4 DESC, f.sname ASC;
END;
$$;

REVOKE ALL ON FUNCTION public.get_ranking() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_ranking() FROM anon;
